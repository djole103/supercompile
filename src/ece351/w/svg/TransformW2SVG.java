...
public final class TransformW2SVG {
	...
	/**
	 * Try changing the value of this flag and see how it changes the
	 * performance of the test harnesses.
	 */
	...
	public static void transform(final WProgram wp, final PrintWriter out) {
		...
		for (final Waveform w : wp.waveforms) {
			...
			for (final String bit : w.bits) {
				...
				if (y_pos == 150) {
					if (bit == "0") {
						out.println(out.format(
								"<line x1=\"%d\" x2=\"%d\" y1=\"%d\" y2=\"%d\"/>",
								x, x, y_pos, y_pos + 50));
						y_pos += 50;
						out.println(out.format(
								"<line x1=\"%d\" x2=\"%d\" y1=\"%d\" y2=\"%d\"/>",
					...
					if (y_pos < y_mid) {
						if (bit == "0") {
							out.println(out
									.format("<line x1=\"%d\" x2=\"%d\" y1=\"%d\" y2=\"%d\"/>",
											x, x, y_pos, y_pos + 100));
							y_pos += 100;
							out.println(out
									.format("<line x1=\"%d\" x2=\"%d\" y1=\"%d\" y2=\"%d\"/>",
